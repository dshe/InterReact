...
namespace InterReact.UnitTests.NoConnect
{
    public class Client_No_Connect_Tests : BaseUnitTest
    {
        ...
        public async Task T01_Cancel()
        {
            ...
            var task = new InterReactBuilder(Logger).BuildAsync(ct);
        ...
        public async Task T02_Timeout()
        {
            ...
            var task = new InterReactBuilder(Logger).BuildAsync(cts.Token);
        ...
        {
            var task = new InterReactBuilder(Logger).SetPort(999).BuildAsync();
