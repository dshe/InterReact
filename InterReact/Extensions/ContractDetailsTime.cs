...
{
    private static readonly LocalTimePattern _timePattern = LocalTimePattern.CreateWithInvariantCulture("HHmm");
    private static readonly LocalDatePattern _datePattern = LocalDatePattern.CreateWithInvariantCulture("yyyyMMdd");
    private readonly ContractDetails _contractDetails;
    private readonly IScheduler _theScheduler;
    ...
    public ContractDetailsTime(ContractDetails contractDetails, IScheduler? scheduler = null)
    {
        ...
        _theScheduler = scheduler ?? Scheduler.Default;
    ...
    private List<ContractDetailsTimeEvent> GetList()
    {
        ...
        foreach ((LocalDateTime start, LocalDateTime end) in GetSessions(_contractDetails.TradingHours))
        {
            list.Add(start, ContractTimeStatus.Trading);
            list.Add(end, ContractTimeStatus.Closed);
        }
        foreach ((LocalDateTime start, LocalDateTime end) in GetSessions(_contractDetails.LiquidHours))
        {
    ...
    private static List<(LocalDateTime start, LocalDateTime end)> GetSessions(string s)
    {
        ...
        foreach (string day in days)
        {
            ...
            LocalDate date = _datePattern.Parse(parts[0]).Value;
        ...
        {
            LocalTime[] times = session.Split('-').Select(t => _timePattern.Parse(t).Value).ToArray();
    ...
    public ContractDetailsTimePeriod? Get() => Get(Instant.FromDateTimeOffset(_theScheduler.Now));
    ...
    private IObservable<ContractDetailsTimePeriod> CreateContractTimeObservable()
    {
        return Observable.Create<ContractDetailsTimePeriod>(observer =>
        {
            ...
            return _theScheduler.Schedule(initialResult.Value.Next.Value.Time.ToDateTimeOffset(), Work);
